...
test('serves files added to the build output before startup', async ({ request }) => {
	const response = await request.get('/added-after-build.txt');
	expect(response.status()).toBe(200);
	expect(response.headers()['etag']).toBeTruthy();
	expect(await response.text()).toBe('added');
});

test('serves files replaced in the build output before startup', async ({ request }) => {
	const response = await request.get('/replaced-after-build.txt', {
	...
	expect(await response.text()).toBe('replaced after build');
});
